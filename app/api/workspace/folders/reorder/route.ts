...
/**
 * Persist sibling folder order after a drag. Body: { slug, orderedIds }.
 * Managers only; one CASE update scoped to the workspace.
 */
export async function POST(req: NextRequest) {
  ...
  await db.execute(sql`
    UPDATE folders SET position = CASE id ${sql.join(cases, sql` `)} ELSE position END
