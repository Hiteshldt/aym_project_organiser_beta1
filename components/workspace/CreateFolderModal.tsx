...
export default function CreateFolderModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) return;
    setSaving(true);
    // New folders default to the Cards view — switch to a Register anytime
    // from the view toggle in the folder header.
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {isSubfolder ? `New subfolder in "${parentFolder!.name}"` : "New folder"}
          </DialogTitle>
        </DialogHeader>
