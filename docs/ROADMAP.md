...
- **Premium effects pass** — an "ember" effects kit (gradient display text, film grain, slow conic shine borders, floating frames, standard card-lift hover), applied with restraint: hero headline + register frame + stat strip, bento tiles, featured pricing glow, FinalCTA grain, editorial footer sign-off, login glow + shine card, and the folder's color dot in the app's content header. All reduced-motion-safe.
- **Brand revamp pass** — landing hero now shows the real register plus a "what your client opens" mini frame; stale ⌘K claims replaced with honest typo-tolerant-search copy; FinalCTA calmed to one accent wash. In-app: dates/indexes in Geist Mono, table links accent-restrained (mute → accent on hover), focus-visible rings, serif wordmark in the top bar, sign-out moved to Settings. Client view: studio name leads the header, pinned items render as a "Start here" strip, share label no longer leaks to clients. **Sticky table header** while scrolling long registers. Type scale + interaction rules codified in BRAND.md/globals.css.
